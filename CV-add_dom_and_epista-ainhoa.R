...
library(lattice)
require(plyr)
library(Matrix) # for nearPD function , postive definitive matrix
library(synbreed) # for kin function
...
pedigree$gener<-c(rep(0, 44),rep(1,dim(pedigree)[1]-44) )

# it could be any of codegene data file, you could use Fikret's data file
load("C:/Users/zhen0001/Desktop/Box Sync/Genomic_selection_Fikret/data/Norway spruce codeGeno data.rda")
...
# to get positive dominance matrix
...
for (trait in names(new.ph.df)[(jump_Nocolumn+1):(jump_Nocolumn+nfold)]) {
  asr <- asreml(fixed = get(trait) ~ 1, 
                random = ~ ped(Genotype_id) + giv(Genotype_idd) ,
...
# for each fold, get the predictions only for the held out observations
#******************* giv(Genotype_idd)_
new.ph.df$ID_add<-paste("ped(Genotype_id)_", new.ph.df$Genotype_id,sep="")    # main
new.ph.df$ID_Dom<-paste("giv(Genotype_idd)_", new.ph.df$Genotype_id,sep="")   # dominance
...
preds<-cbind(tempt, new.ph.df[, c("Genotype_id")], as.matrix(A_inbreeding[45:length(A_inbreeding)]))
colnames(preds)<-c( "solution_add","std error_add", "z ratio_add",  "solution_dom","std error_dom", "z ratio_dom","Genotype_id","inbreeding" ) 
...
print(c(a1, b1, a2, b2)) # additive predivitive abiligy, standard eroror, and domiannce model preditive ability 
...
# for each fold, get the predictions only for the held out observations
#******************* giv(Genotype_idd)_
new.ph.df$ID_add<-paste("ped(Genotype_id)_", new.ph.df$Genotype_id,sep="")    # additive
...
a1<-round(mean(temp$COR),3);   b1<-round(SE_Accuracy_A,3)
print(c(a1, b1))  # a1)  predictive ability and b1 standard errror
