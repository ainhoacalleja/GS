...
tabRes
write.csv(tabRes, file="tabres_BRR2_d30.csv", row.names = TRUE)
write.csv(rawdf,file = "10fold_BRR2_d30.csv", row.names = TRUE )
...
###################### CV for different number of SNPs #############

#######Number of SNP to select
NosubsetSNP=10 # 10, 20, 50, 100, 200, 500, 1000, 2000, 3000, 4000, 5000, 6000, 7000, #8719
...
# trait for cv
EBV<-phenot2[, "EBV_h30"]           
tt<-as.data.frame(phenot2[, "ht30.1"]) 
colnames(tt)<-c("ht30.1")           
...
# Print final SUMMARY stats     
tabRes
write.csv(tabRes, file="10SNP_res_BRR2_Ht30.csv", row.names = TRUE)
write.csv(rawdf,file = "10SNP_BRR2_Ht30.csv", row.names = TRUE )
...
# 
# write.csv(results.average, file="CV6_BL_ht30_EM.csv", row.names = TRUE)
# 
# 
# # Sample ebv and GEBV 
...
# tail(rawdf)
# str(rawdf)
# write.csv(rawdf, file="TS_EM_CV6_BL_ht30.csv", row.names=TRUE)
